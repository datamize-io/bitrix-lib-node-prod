import { ConnectorBuilder, ConnectorRegisterBuilder, ConnectorDatasetBuilder } from "../../builders/im/ConnectorBuilder.builder.js";
...
export class Connector extends ConnectorBuilder {
   ...
   */
  async register(registerBuilder: ConnectorRegister | ConnectorRegisterBuilder) {
   ...
   * @returns Resultado da requisição PATCH
   */
  async unregister(id: string | number) {
   ...
   */
  async activate(connectorId: string, lineId: string | number, active: "Y" | "N") {
    const payload = {
   ...
   * @returns Resultado da requisição PATCH
   */
  async setConnectorDataset(datasetBuilder: ConnectorDataSet) {
   ...
   */
  async sendMessages(connectorId: string, lineId: string | number, messages: ConnectorSendMessagesInterface) {
    const payload = {
      CONNECTOR: connectorId,
      LINE: lineId,
