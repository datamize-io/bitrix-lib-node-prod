...
import { OpenLineChatInterface } from "../../interfaces/im/OpenLineChatInterface.interface.js";
...
export class OpenLineChat extends OpenLineChatBuilder {
   ...
   * ```
   */
  async getChatsByEntityId(entity: "lead" | "deal" | "contact" | "company", id: string | number, onlyActiveChats: "Y" | "N" = "Y") {
   ...
   * ```
   */
  async getLastChatIdByEntityId(entity: "LEAD" | "DEAL" | "CONTACT" | "COMPANY", id: string | number) {
   ...
   * ```
   */
  async addUser(entity: "LEAD" | "DEAL" | "CONTACT" | "COMPANY", id: string | number, userId: string | number, chatId?: string | number) {
   ...
   * ```
   */
  async removeUser(entity: "LEAD" | "DEAL" | "CONTACT" | "COMPANY", id: string | number, userId: string | number, chatId?: string | number) {
    chatId = chatId || this.getData()["CHAT_ID"];
   ...
   * ```
   */
  async collect(params?: any | null, method?: string | null, collectField: string | null = "result"): Promise<any | this> {
