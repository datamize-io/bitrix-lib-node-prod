 ...
 */
export class OpenLineDialog extends OpenLineDialogBuilder implements OpenLineDialogInterface {
  /**
   ...
   */
  async getHistory(chatId: string | number, sessionId: string | number) {
   ...
   */
  async start(chatId: string | number) {
   ...
   */
  async startByMessage(chatId: string | number, messageId: string | number): Promise<boolean> {
   ...
   */
  async startByUserCode(userCode: UserCodeBuilder) {
    const stringUserCode: string = userCode.build();
   ...
   */
  async join(chatId: string | number) {
   ...
   * @returns Resultado da operação.
   */
  async intercept(chatId: string | number) {
   ...
   * @returns Resultado da operação.
   */
  async pin(chatId: string | number, enablePin: "Y" | "N") {
    return await this.requestAndPatch("imopenlines.session.intercept", {
      CHAT_ID: chatId,
      ACTIVATE: enablePin,
    });
   ...
   * @returns Resultado da operação.
   */
  async silent(chatId: string | number, enablePin: "Y" | "N") {
   ...
   */
  async rating(sessionId: string | number, ratingNumber?: 1 | 2 | 3 | 4 | 5, comment?: string): Promise<boolean> {
   ...
   */
  async createLeadFromDialog(chatId: string | number): Promise<boolean> {
   ...
   */
  async getByChatId(id: string | number) {
   ...
   * @returns Dados do diálogo.
   */
  async getByDialogId(id: string | number) {
    return await this.requestAndPatch("imopenlines.dialog.get", {
      DIALOG_ID: id,
    });
  }
   ...
   * @returns Dados do diálogo.
   */
  async getBySessionId(id: string | number) {
    return await this.requestAndPatch("imopenlines.dialog.get", {
      SESSION_ID: id,
    });
  }
   ...
   * @returns Dados do diálogo.
   */
  async getByUserCode(userCode: UserCodeBuilder) {
    const stringUserCode: string = userCode.build();
    return await this.requestAndPatch("imopenlines.dialog.get", {
      USER_CODE: stringUserCode,
    });
