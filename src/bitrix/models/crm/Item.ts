...
import { Activity, Comment, Contact, ContactField, CrmType } from "./index.js";
...
export class Item extends ItemBuilder {
  ...
  async insertComment(comment: string) {
  ...
  async transferAllActivitiesTo(newOwnerId: string | number) {
    try {
      ...
      await activities.getData().forEach(async (activity: Activity) => {
  ...
  async transferAllCommentsTo(newOwnerId: string | number) {
    ...
    comments.getData().forEach(async (comment: Comment) => {
      ...
      const insertResponse = newCommentIntoNewContact.insert(newCommentIntoNewContact.getData()).catch((error: Error) => {
        throw Error("Erro ao transferir comentário:" + error.message);
      });

      insertResponse.then(async (newComment: Comment) => {
  ...
  async transferEntitiesTo(newOwnerId: string | number, chooseEntities: number[] = []) {
    ...
    if (chooseEntities.length == 0) {
      ...
      const systemEntities: number[] = [1, 2, 3, 4];
    ...
    chooseEntities.forEach(async (entityTypeId: any) => {
  ...
  async transferAllFieldsTo(newOwnerId: any) {
    const itemData = this.getData();
    ...
    fieldKeys.forEach(function (fieldKey) {
      ...
      if (("" + oldFieldValue).trim() !== ("" + newFieldValue).trim() && newFieldValue) {
        ...
        if (translatedField.type == "enumeration") {
          const option = translatedField.items.find((optionObject: any) => optionObject.ID == newFieldValue);
        ...
        if (Array.isArray(oldFieldValue) && Array.isArray(newFieldValue) && systemKeys.includes(fieldKey)) {
          ...
          newFieldValueText = newFieldValue.map((deal: any) => deal.VALUE).join("\n- ");
  ...
  async clearAllFieldsOfItem() {
    ...
    const emptyValuesData: { [key: string]: any } = {};
    ...
    Object.keys(itemData).forEach(function (dataKey) {
      ...
      if (!itemData[dataKey]) return;
      if (fieldSetting?.isReadOnly !== false) return;
